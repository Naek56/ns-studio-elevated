/**
 * Génère le contenu du fichier kairos-tracker.js pour un client donné.
 * Le script est autonome, respecte le consentement RGPD, et envoie les events
 * vers l'endpoint d'ingestion `${origin}/api/track`.
 */
export function buildTracker(clientId: string, origin: string): string {
  const endpoint = `${origin.replace(/\/$/, "")}/api/track`;

  // IIFE autonome. CLIENT_ID et ENDPOINT sont injectés littéralement.
