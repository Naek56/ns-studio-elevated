...
export default function ChatTab({ client }: { client: KairosClient }) {
  ...
  useEffect(() => {
    (async () => {
      const res = await fetch(`/api/clients/${client.client_id}/chat`, {
        cache: "no-store",
      });
      if (res.ok) {
        ...
        {messages.length === 0 && !sending && (
          <div className="flex h-full flex-col items-center justify-center text-center">
            <p className="font-display text-3xl font-light text-white">
              Discutez avec Kairos
            </p>
            <p className="mt-3 max-w-md font-mono text-sm text-kairos-muted">
              Kairos connaît {client.nom} : ses 3 derniers rapports, ses données
              des 7 derniers jours et son profil. Posez votre question.
            </p>
          </div>
        )}
