...
import type { KairosClient, LiveEvent } from "@/lib/types";
...
function applyEvent(map: Map<string, Session>, ev: LiveEvent) {
  ...
  if (existing) {
    ...
    if (ev.page && (ev.type === "pageview" || ev.type === "scroll")) existing.page = ev.page;
  } else {
    map.set(sid, { id: sid, page: ev.page || "/", firstSeen: ts, lastSeen: ts });
  }
}
...
export default function LiveTab({ client }: { client: KairosClient }) {
  ...
  useEffect(() => {
    ...
    const refresh = () => {
      ...
      setSessions(Array.from(mapRef.current.values()).sort((a, b) => b.lastSeen - a.lastSeen));
    };
    ...
    (async () => {
      ...
      if (cancelled) return;
      for (const ev of (data || []) as LiveEvent[]) applyEvent(mapRef.current, ev);
      refresh();
    })();
    ...
    channel = supabase
      ...
      .on(
        ...
        (payload) => {
          applyEvent(mapRef.current, payload.new as LiveEvent);
          refresh();
        }
      )
    ...
    return (
      <div className="mx-auto max-w-4xl px-8 py-8">
        <p className="font-mono text-sm text-kairos-red">
          Supabase n'est pas configuré (VITE_SUPABASE_URL / VITE_SUPABASE_PUBLISHABLE_KEY).
          Le temps réel est indisponible.
        </p>
      </div>
    );
  }
    ...
    <div className="mx-auto max-w-4xl px-8 py-8">
      ...
      <div className="mt-8 flex items-end gap-4 rounded-lg border border-kairos-border bg-kairos-panel p-6">
        ...
        <span className="mb-2 font-mono text-sm text-kairos-muted">
          visiteur{sessions.length > 1 ? "s" : ""} actif{sessions.length > 1 ? "s" : ""}
        </span>
      </div>
          ...
          <ul className="space-y-1.5">
            {sessions.map((s) => (
              ...
              >
                ...
                <span className="text-kairos-muted">{formatDuration(now - s.firstSeen)}</span>
              </li>
            ))}
          </ul>
        )}
