    ...
    <div className="mx-auto max-w-4xl px-8 py-8">
      ...
      <div className="mt-6">
        ...
        <div className="mt-3 flex items-center justify-between">
          {error ? <p className="font-mono text-xs text-kairos-red">{error}</p> : <span />}
          <button
            onClick={analyze}
            disabled={running}
        ...
        <>
          <div className="mt-8 rounded-lg border border-kairos-border bg-kairos-bg p-6">
            <p className="mb-4 font-mono text-[11px] uppercase tracking-widest text-kairos-muted">
              Analyse du {formatDate(selected.created_at)} · {(selected.urls || []).length} concurrent
                ...
                {analyses.map((a) => (
                  <li key={a.id}>
                    ...
                    >
                      ...
                      <span className="text-kairos-muted">{(a.urls || []).length} URL</span>
                    </button>
                  </li>
                ))}
              </ul>
