...
export function summarizeEvents(events: RawEvent[], periodeJours = 7): EventsSummary {
  ...
  const topList = (m: Map<string, number>) =>
    Array.from(m.entries())
      .sort((a, b) => b[1] - a[1])
      .slice(0, 8)
      .map(([element, occurrences]) => ({ element, occurrences }));
  ...
  return {
    ...
    tauxRebondEstime,
    pages,
    elementsRageClick: topList(rageElements),
    elementsCliques: topList(clickElements),
  };
}
