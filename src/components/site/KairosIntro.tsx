...
export default function KairosIntro() {
  ...
  return (
    <section ref={root} id="kairos" className="relative overflow-hidden px-6 py-28 md:px-10 md:py-40">
      <div className="mx-auto max-w-[1200px]">
        <div className="k-head max-w-4xl opacity-0">
          <p className="type-body text-xs font-semibold uppercase tracking-[0.32em] text-neutral-600">Le cerveau</p>
          <h2 className="type-strong mt-6" style={{ fontSize: "clamp(2.6rem, 7vw, 5.6rem)" }}>
            Kairos change ça.
          </h2>
        </div>

        <div className="k-grid mt-16 grid grid-cols-1 gap-5 sm:grid-cols-2 md:mt-24">
          {ROLES.map((r) => (
            <article key={r.t} className="k-card paper paper-grain relative overflow-hidden rounded-2xl p-8 opacity-0 md:p-10">
              <h3 className="type-body relative z-10 text-2xl font-semibold text-neutral-900 md:text-3xl">{r.t}</h3>
              <p className="type-body relative z-10 mt-4 max-w-md text-base leading-relaxed text-neutral-700">{r.d}</p>
            </article>
          ))}
        </div>
      </div>
