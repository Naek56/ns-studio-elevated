...
export default function KairosIntro() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      gsap.fromTo(".k-card", { opacity: 0, y: 54, scale: 0.94 }, {
        opacity: 1, y: 0, scale: 1, duration: 0.9, stagger: 0.12, ease: "power3.out",
        scrollTrigger: { trigger: ".k-grid", start: "top 80%", once: true },
      });
    }, root);
    return () => ctx.revert();
  }, []);
  ...
  return (
    <section ref={root} id="kairos" className="relative overflow-hidden px-6 py-16 text-center md:px-10 md:py-24">
      <div className="mx-auto max-w-[1200px]">
        <div className="k-head mx-auto max-w-4xl opacity-0">
          ...
          <h2 className="type-strong mt-4" style={{ fontSize: "clamp(1.9rem, 5vw, 3.6rem)" }}>
            Kairos change ça.
          </h2>
        </div>

        <div className="k-grid mx-auto mt-12 grid max-w-[1000px] grid-cols-1 gap-4 sm:grid-cols-2 md:mt-14">
          {ROLES.map((r) => (
            <article
              key={r.t}
              className="k-card relative overflow-hidden rounded-2xl border border-white/15 p-6 text-center opacity-0 md:p-8"
              style={{ background: "rgba(10,20,40,0.32)", backdropFilter: "blur(10px)", WebkitBackdropFilter: "blur(10px)" }}
            >
              <h3 className="type-body relative z-10 text-xl font-semibold text-white md:text-2xl">{r.t}</h3>
              <p className="type-body relative z-10 mx-auto mt-3 max-w-md text-sm leading-relaxed text-white/75">{r.d}</p>
            </article>
          ))}
        </div>
      </div>
